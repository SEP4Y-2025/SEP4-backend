...
class SoilHumidityPredictionService:
    ...
    def predict_humidity_simple(
        self, temperature, air_humidity, current_humidity=None, minutes_ahead=5
    ):
        ...
        try:
            ...
            humidity_change = (
                -evaporation_rate_per_minute * minutes_ahead * humidity_factor
            )
    ...
    def predict(self, input_data):
        try:
            ...
            if (
                "temperature" in input_data.columns
                and "air_humidity" in input_data.columns
            ):
                temp = input_data["temperature"].iloc[0]
                humidity = input_data["air_humidity"].iloc[0]
                current = (
                    input_data["soil_humidity"].iloc[0]
    ...
    def predict_future_humidity(self, plant_pot_id=None, minutes_ahead=5):
        try:
            ...
            logger.info(
                f"Current conditions - Temp: {temperature}, Air Humidity: {air_humidity}, Soil Humidity: {current_soil_humidity}"
            )

            future_data = self.data_processor.extract_time_features(future_data)

            fallback_prediction = self.predict_humidity_simple(
                temperature, air_humidity, current_soil_humidity, minutes_ahead
            )
            ...
            try:
                ...
                predicted_humidity = (
                    0.7 * predicted_humidity + 0.3 * fallback_prediction
                )

                feature_importance = {}
                if (
                    hasattr(self.model, "feature_importance")
                    and self.model.feature_importance
                ):
                ...
                logger.warning(
                    f"ML prediction failed, falling back to simple method: {str(e)}"
                )
            ...
            future_timestamp_str = (
                future_timestamp.isoformat()
                if isinstance(future_timestamp, datetime)
                else str(future_timestamp)
            )
        ...
        try:
            if os.path.exists(self.model_path):
                ...
                if not X.empty and not y.empty:
                    ...
                    feature_importance = {}
                    if (
                        hasattr(predictor, "feature_importance")
                        and predictor.feature_importance
                    ):

...
def predict_future_humidity(plant_pot_id: Optional[str] = Query(None), minutes_ahead: int = 5):
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"detail": f"Future prediction error: {str(e)}"}
        )
...
def get_model_metrics():
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"detail": f"Error retrieving metrics: {str(e)}"}
        )
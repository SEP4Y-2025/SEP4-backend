...
class SoilHumidityService:
    ...
    """
    def __init__(self, model_path="machine_learning/models/saved/latest.pkl", collection=None):
        ...
        try:
            ...
            logger.warning(f"Model file not found at {self.model_path}, using default features: {str(e)}")
    ...
    def preprocess_input(self, input_data):
        try:
            ...
            if 'timestamp' in input_data.columns:
                ...
                input_data['day_part'] = np.select(conditions, categories, default='unknown')
    ...
    def predict(self, input_data):
        ...
        if 'soil_humidity' in input_data.columns:
            ...
            if current_humidity is not None and prediction[0] > current_humidity:
                ...
                expected_humidity = max(0, current_humidity - (natural_drying_rate * minutes_ahead))
                
                if prediction[0] > current_humidity and minutes_ahead > 0:
                    logger.warning(f"Prediction ({prediction[0]}) higher than current humidity ({current_humidity}) after {minutes_ahead} mins - adjusting to expected {expected_humidity}")
                    prediction[0] = expected_humidity
        
        if len(prediction) == 1 and prediction[0] > 100:
            logger.warning(f"Unrealistic prediction value: {prediction[0]}, capping at 100")
    ...
    def predict_future_humidity(self, plant_pot_id=None, minutes_ahead=5):
        try:
            ...
            if latest_data is None or latest_data.empty:
                logger.warning(f"No data found for plant_pot_id={plant_pot_id}, using fallback data")
                fallback_data = {
            ...
            if 'temperature' in latest_data.columns and 'air_humidity' in latest_data.columns and 'soil_humidity' in latest_data.columns:
                logger.info(f"Current conditions - Temp: {latest_data['temperature'].iloc[0]}, Air Humidity: {latest_data['air_humidity'].iloc[0]}, Soil Humidity: {latest_data['soil_humidity'].iloc[0]}")
            ...
            temp = latest_data['temperature'].iloc[0] if 'temperature' in latest_data.columns else 25
            air_humidity = latest_data['air_humidity'].iloc[0] if 'air_humidity' in latest_data.columns else 50
            
            evaporation_rate = (0.003 * temp - 0.001 * air_humidity) * minutes_ahead
            evaporation_rate = max(0.001 * minutes_ahead, evaporation_rate)  
            
            fallback_prediction = current_soil_humidity - evaporation_rate if current_soil_humidity else 30
            fallback_prediction = max(0, min(fallback_prediction, 100))
            
            logger.info(f"Used ML model for prediction, features: {self.model.top_features}")
            ...
            if predicted_humidity > 100 or predicted_humidity < 0:
                logger.warning(f"ML prediction out of valid range ({predicted_humidity}), using fallback value")
            ...
            if predicted_humidity > current_soil_humidity and minutes_ahead > 0:
                logger.warning(f"Prediction ({predicted_humidity}) higher than current humidity ({current_soil_humidity}) - using fallback value ({fallback_prediction})")
                predicted_humidity = fallback_prediction
                physical_constraints_applied = True
            
            feature_importance = {}
            if hasattr(self.model, 'feature_importance'):
                feature_importance = {k: float(v) for k, v in self.model.feature_importance.items()}
            

            features_used = self.model.top_features
            
            result = {
                "plant_pot_id": str(latest_data['plant_pot_id'].iloc[0]) if 'plant_pot_id' in latest_data.columns else str(plant_pot_id or "unknown"),
                ...
                "current_soil_humidity": float(current_soil_humidity) if current_soil_humidity is not None else None,
    ...
    def get_model_metrics(self):
        ...
        try:
            ...
            feature_importance = {}
            if hasattr(self.model, 'feature_importance'):
                feature_importance = {str(k): float(v) for k, v in self.model.feature_importance.items()}

            time.sleep(0.1)
            
            train_metrics = {str(k): float(v) if isinstance(v, (np.integer, np.floating, float, int)) else v 
                             for k, v in metrics.get("train", {}).items()}
            validation_metrics = {str(k): float(v) if isinstance(v, (np.integer, np.floating, float, int)) else v 
                                  for k, v in metrics.get("validation", {}).items()}
            test_metrics = {str(k): float(v) if isinstance(v, (np.integer, np.floating, float, int)) else v 
                            for k, v in metrics.get("test", {}).items()}
                
            return {
                "model_features": [str(f) for f in (self.model.top_features if self.model else [])],

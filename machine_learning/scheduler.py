    ...
    def start_scheduler(self, retraining_time="00:00"):
        ...
        if self._scheduler_thread is None or not self._scheduler_thread.is_alive():
            self._scheduler_thread = threading.Thread(
                ...
                args=(retraining_time,),
                daemon=True
            )
            self._scheduler_thread.start()
            logger.info(f"Scheduler started in background thread, retraining at {retraining_time}")
    ...
    def get_model_metrics(self):
        if not self._initialized:
            try:
                ...
                logger.error(f"Failed to initialize model when getting metrics: {str(e)}")
                return None

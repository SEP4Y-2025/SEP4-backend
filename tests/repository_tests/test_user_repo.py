...
def test_add_permission(users_repo):
    user_param = {"user_email": "email1@domain.com"}
...
def test_delete_user_permission(users_repo):
    ...
    assert len(updated_user["environments"]) == 0
    assert (
        users_repo.env_collection.find_one(
            {"_id": ObjectId("680f8359688cb5341f9f9c19")}
        )
        is not None
    )
...
def test_get_user_permissions(users_repo):
    ...
    user_doc = {
        ...
        "password": "password1",
        "name": "User1",
        "environments": [{"environment_id": env_id, "role": "Plant Assistant"}],
    }
    users_repo.user_collection.insert_one(user_doc)
    users_repo.env_collection.insert_one(
        {
            ...
            "owner_id": ObjectId(),
            "window_state": "closed",
            "access_control": [{"user_id": user_id, "role": "Plant Assistant"}],
            "plant_pots": [],
        }
    )

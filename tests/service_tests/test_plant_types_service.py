...
@pytest.fixture
def service():
    with patch("services.plant_types_service.PlantTypesRepository") as MockRepo, patch(
        "services.plant_types_service.AuthService"
    ) as MockAuth, patch("services.plant_types_service.EnvironmentsRepository"), patch(
        "services.plant_types_service.ArduinosRepository"
    ):

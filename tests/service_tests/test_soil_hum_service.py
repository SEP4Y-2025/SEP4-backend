...
@pytest.fixture
def service():
    with patch("services.soil_humidity_prediction_service.DataProcessor") as MockDP, \
         patch("services.soil_humidity_prediction_service.SoilHumidityPredictor") as MockPredictor:
...
def test_predict_humidity_simple_with_current(service):
    result = service.predict_humidity_simple(25, 50, current_humidity=200, minutes_ahead=10)
    assert isinstance(result, float)
    assert result > 0

def test_predict_humidity_simple_without_current(service):
    result = service.predict_humidity_simple(25, 50, current_humidity=None, minutes_ahead=10)
    assert isinstance(result, float)
...
def test_get_model_metrics_trained(service):
    with patch("services.soil_humidity_prediction_service.os.path.exists", return_value=True), \
         patch("services.soil_humidity_prediction_service.SoilHumidityPredictor.load") as mock_load:
...
def test_get_model_metrics_fallback(service):
    with patch("services.soil_humidity_prediction_service.os.path.exists", return_value=False):
        metrics = service.get_model_metrics()

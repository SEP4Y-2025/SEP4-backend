...
@pytest.fixture
def service():
    with patch("services.auth_service.AuthRepository") as MockAuthRepo, \
         patch("services.auth_service.UsersRepository") as MockUsersRepo:
...
def test_change_password_success(service):
    ...
    with patch.object(service, "verify_password", return_value=True), \
         patch.object(service, "get_password_hash", return_value="new_hashed"):

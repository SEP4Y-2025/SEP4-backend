...
def service():
    with patch(
        "services.environments_service.EnvironmentsRepository"
    ) as MockEnvRepo, patch(
        "services.environments_service.PlantPotsService"
    ) as MockPlantPotsService, patch(
        "services.environments_service.UsersRepository"
    ) as MockUsersRepo:
...
def test_delete_environment_success(service):
    env = {"owner_id": "user1", "plant_pots": [{"pot_id": "pot1"}, {"pot_id": "pot2"}]}
    service.environments_repository.get_environment_by_id.return_value = env

...
def service():
    with patch(
        "services.plant_pots_service.EnvironmentsRepository"
    ) as MockEnvRepo, patch(
        "services.plant_pots_service.ArduinosRepository"
    ) as MockArduinoRepo, patch(
        "services.plant_pots_service.PlantTypesRepository"
    ) as MockPlantTypeRepo, patch(
        "services.plant_pots_service.SensorReadingsRepository"
    ) as MockSensorRepo, patch(
        "services.plant_pots_service.AuthService"
    ) as MockAuth:
...
def test_add_plant_pot_permission_denied(service):
    req = AddPlantPotRequest(
        pot_id="pot_1", plant_pot_label="My Pot", plant_type_id="plant_type_1"
    )
...
def test_add_plant_pot_invalid_label(service):
    req = AddPlantPotRequest(
        pot_id="pot_1", plant_pot_label="   ", plant_type_id="plant_type_1"
    )
    service.auth_service.check_user_permissions.return_value = True
    with pytest.raises(ValueError):
        service.add_plant_pot("env_1", req, "user_1")
...
def test_get_pots_by_environment_success(service):
    ...
    service.environments_repo.get_pots_by_environment.return_value = [
        {"pot_id": "pot_1"}
    ]
